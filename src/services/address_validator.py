...
class AddressValidator:
    ...
    def __init__(self,
        ...
        if not self.client_id or not self.client_secret:
            logger.error("USPS_CLIENT_ID or USPS_CLIENT_SECRET not provided or found in environment variables.")
            raise ValueError("USPS Client ID and Client Secret are required.")

        self.base_url = "https://apis-tem.usps.com" if self.use_test_env else "https://apis.usps.com"
        ...
        logger.info(f"AddressValidator initialized. Using {'Test' if self.use_test_env else 'Production'} USPS API environment.")
    ...
    async def _get_access_token(self) -> Optional[str]:
        ...
        async with self._lock: # Ensure only one coroutine attempts to refresh the token at a time
            ...
            try:
                async with session.post(self.oauth_url, json=payload, headers=headers) as response:
                    response_data = await response.json()
                    if response.status == 200 and "access_token" in response_data:
                        self._access_token = response_data["access_token"]
                        expires_in = int(response_data.get("expires_in", 3599)) # Default to 59 mins (3540s) if not specified
                        self._token_expires_at = time.time() + expires_in - 60 # Subtract 1 min buffer
                        logger.info(f"Successfully obtained new access token. Expires in {expires_in}s.")
                        return self._access_token
                    else:
                        error_detail = response_data.get("error_description") or response_data.get("error") or str(response_data)
                        logger.error(f"Failed to obtain access token. Status: {response.status}, Response: {error_detail}")
    ...
    async def validate_address(self,
                               ...
                               zip5: str,
                               zip4: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if zip4: # The API example includes ZIPPlus4 in the request for address validation.
        ...
        try:
            logger.debug(f"Sending address validation request to USPS: {params}")
            async with session.get(self.address_api_url, params=params, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.debug(f"USPS Address API response: {data}")
                    ...
                    res_street2 = addr_info.get("secondaryAddress", street2 if street2 else "") # Ensure empty string not None
                    ...
                    validated_addr_payload = {
                        "street1": res_street1,
                        "street2": res_street2 if res_street2 else None, # Convert empty string back to None if it was initially
                        "city": res_city,
                        "state": res_state,
                        "zip5": res_zip5,
                    ...
                    dpv_confirmation = data.get("addressAdditionalInfo", {}).get("DPVConfirmation", "N")
                    ...
                    if dpv_confirmation == "Y": # Address is DPV confirmed as deliverable.
                        if input_addr_str.upper() != output_addr_str.upper() or data.get("addressCorrections"):
                            return {
                                "status": "VALID_WITH_CHANGES",
                                "reason": "Address validated with corrections. " + (data.get("addressCorrections", [{}])[0].get("correctionText", "") if data.get("addressCorrections") else ""),
                    ...
                    elif dpv_confirmation == "S": # Confirmed, but secondary number missing / incorrect.
                         return {
                            "status": "VALID_WITH_ISSUES", # Or "AMBIGUOUS" if secondary is crucial
                            "reason": "Address confirmed, but requires attention to the secondary address unit (e.g., apartment, suite). " + (data.get("addressCorrections", [{}])[0].get("correctionText", "") if data.get("addressCorrections") else "Please verify the apartment or suite number."),
                            "validated_address": validated_addr_payload
                        }
                    elif dpv_confirmation == "D": # Confirmed, but primary number missing.
                        return {
                            "status": "VALID_WITH_ISSUES",
                            "reason": "Address confirmed, but the primary street number is missing or invalid. " + (data.get("addressCorrections", [{}])[0].get("correctionText", "") if data.get("addressCorrections") else "Please verify the street number."),
                            "validated_address": validated_addr_payload
                        }
                        ...
                        reason_parts = ["Address could not be validated as deliverable."]
                        if data.get("addressAdditionalInfo", {}).get("DPVFootnotes"):
                            reason_parts.append("Footnotes: " + data.get("addressAdditionalInfo", {}).get("DPVFootnotes"))
                        if data.get("addressCorrections"):
                             reason_parts.append("Corrections: " + data.get("addressCorrections", [{}])[0].get("correctionText", ""))
                ...
                elif response.status == 400: # Bad Request, often means invalid input format or unfindable
                    error_data = await response.json()
                    error_messages = [err.get("message", "Unknown error detail") for err in error_data.get("errors", [])]
                    logger.warning(f"USPS Address API Bad Request (400): {error_messages}. Input: {params}")
                    return {
                        "status": "INVALID", # Treat as invalid if API says bad request for address
                        "reason": "USPS API reported an issue with the address provided: " + "; ".join(error_messages) if error_messages else "The address could not be found or was badly formatted.",
                        "validated_address": None
                    }
                elif response.status == 401: # Unauthorized - token issue
                     logger.error(f"USPS Address API Unauthorized (401). Token might be invalid or expired.")
                    ...
                    logger.error(f"USPS Address API request failed. Status: {response.status}, Response: {error_text}")
                    return {
                        "status": "API_ERROR",
            ...
            return {"status": "API_ERROR", "reason": f"Network error during address validation: {e}", "validated_address": None}
        except Exception as e:
            logger.error(f"Unexpected error during address validation: {e}")
            return {"status": "ERROR", "reason": f"An unexpected error occurred: {e}", "validated_address": None}
